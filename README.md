...
The experimental evidence and reviewed reflection are published at [jennawang1121/class4-custom-llm](https://github.com/jennawang1121/class4-custom-llm). [Public-access checks](evidence/public_access_check.json) verified 17 downloads without authentication, including both executed notebooks, model files, result ZIPs and four eval result sets. The signed-out README and both loss plots rendered correctly. Both native GitHub notebook previews eventually rendered while signed out, including completion records, final losses/evals, vector outputs and SVG figures. The readable exports remain available as backups. The repository URL still needs to be submitted through the course portal. The benchmark percentage is not the assignment grade.
